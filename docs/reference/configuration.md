...
- packageOptions.rollup.requireReturnsDefault | `string | boolean` (default: 'auto') - This option allows setting of the @rollup/plugin-commonjs option `requireReturnsDefault`. It affects how commonjs modules with default exports are imported. See [@rollup/plugin-commonjs documentation](https://github.com/rollup/plugins/tree/master/packages/commonjs/#requirereturnsdefault) for more. Note: Setting to `preferred` instead of `auto` fixes a common issue where snowpack will fail to build when certain common packages, notably `call-bind`, are included in the dependency tree.
