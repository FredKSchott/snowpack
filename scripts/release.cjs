...
function generateNewChangelog(pkgFolder) {
    return `# Changelog\n\n> *For older releses, check our curated [release update thread](https://github.com/snowpackjs/snowpack/discussions/1183) or the raw [commit history](https://github.com/snowpackjs/snowpack/commits/main/${path.normalize(pkgFolder)}).*`;
}

function generateChangelogUpdate(dir, newTag, oldTag) {
    let {stdout} = execa.sync('git', ['log', `HEAD...${oldTag}`, `--abbrev-commit`, `--format=* %h - %s <%an>`, dir], {cwd: dir});
...
module.exports = function release(pkgFolder, tag, bump) {
    ...
    console.log(execa.sync('npm', ['publish', '--tag', tag, '--dry-run'], {cwd: dir}));
    console.log(execa.sync('git', ['push', '--tags', '--dry-run'], {cwd: dir}));
}