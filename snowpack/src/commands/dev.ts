...
export async function startServer(
  ...
  async function loadUrl(
    ...
    if (reqPath.startsWith(PACKAGE_LINK_PATH_PREFIX)) {
      ...
      const fileStat = await fs.stat(symlinkResourceLoc).catch(() => null);
      if (!fileStat) {
        throw new NotFoundError(reqPath, [symlinkResourceLoc]);
      }
      ...
      if (!symlinkDirectories.has(symlinkResourceDirectory)) {
        ...
        for (const f of glob.sync(path.join(symlinkResourceDirectory, '*'), {
          ...
          fileToUrlMapping.add(
            f,
            getBuiltFileUrls(f, config).map((u) => path.posix.join(PACKAGE_LINK_PATH_PREFIX, u)),
          );
        }
      }
      foundFile = {
        loc: fileToUrlMapping.key(reqPath)!,
