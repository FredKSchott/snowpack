...
import {InstallTarget, resolveEntrypoint} from 'esinstall';
...
export default {
  async load(id: string, isSSR: boolean) {
    ...
    packageCode = await transformFileImports({type, contents: packageCode}, async (spec) => {
      ...
      // TODO: Can we support real tree-shaking here?
      imports.push(
        createInstallTarget(
    ...
    if (installDirectoryHash === 'v1') {
      ...
      logger.info(
        `${colors.bold(
          ...
          'will be reused for the lifetime of your project. Please wait while we prepare your dependencies...',
      );
    }
  ...
  async resolvePackageImport(source: string, spec: string, _config: SnowpackConfig) {
    ...
    const [newImportMap, loadedFile] = await inProgressBuilds.add(
      async (): Promise<[ImportMap, Buffer]> => {
        ...
        logger.info(colors.yellow(`- import "${spec}"...`));
        ...
        logger.debug(colors.yellow(`- import "${spec}"... DONE`));
        if (needsSsrBuild) {
          logger.info(colors.yellow(`- import "${spec}" (ssr)...`));
          ...
          logger.debug(colors.yellow(`- import "${spec}" (ssr)... DONE`));
        }
