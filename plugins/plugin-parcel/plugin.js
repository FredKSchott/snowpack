...
module.exports = function parcelBundlePlugin(config, options) {
  return {
    ...
    async bundle({srcDirectory, destDirectory, log, jsFilePaths}) {
      ...
      await fs.unlink(path.join(srcDirectory, 'postcss.config.js')).catch((/* ignore */) => null);
      await fs.unlink(path.join(srcDirectory, '.postcssrc')).catch((/* ignore */) => null);
      await fs.unlink(path.join(srcDirectory, '.postcssrc.js')).catch((/* ignore */) => null);
