...
describe('@snowpack/plugin-build-svelte', () => {
  ...
  it('builds source svelte files as expected', () => {
    ...
    expect(fs.readFileSync(jsLoc, 'utf-8')).toContain(`import { refresh, comment, camera } from "../_snowpack/pkg/svelte-awesome/icons.js";`); // file has expected imports
  ...
  it('builds package svelte files as expected', () => {
    ...
    expect(fs.readFileSync(path.join(cwd, '_snowpack', 'pkg', 'svelte-awesome', 'icons.js'), 'utf-8')).toMatchSnapshot('svelte-awesome treeshaking'); // import exists, and was tree-shaken
  });
});
